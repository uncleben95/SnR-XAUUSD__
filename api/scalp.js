...
export default async function handler(req, res) {
  ...
  const AI_STATE_KEY = `${CACHE_PREFIX}:ai:last-structure`;
  const CHOCH_STATE_KEY = `${CACHE_PREFIX}:push:last-choch`;
...
Rules:
- The technical engine has just evaluated a CLOSED M5/M15 candle.
- A structure event is supplied. Treat the broken swing as a possible retest level, not an automatic entry.
- If M5 and M15 are not aligned, the answer MUST be WAIT_RETRACE.
- Do NOT blindly approve any setup.
...
Market data:\n${JSON.stringify(context)}`;
    ...
    try {
      ...
      let decision = ["ENTRY_VALID", "WAIT_RETRACE", "AVOID"].includes(parsed?.decision)
        ? parsed.decision
        : "WAIT_RETRACE";

      // Hard safety gates: AI may never turn a non-aligned/non-structure state
      // into an entry signal.
      if (!context?.structureTrigger?.confirmed ||
          !["CHOCH","BOS"].includes(context.structureTrigger.type) ||
          context.direction !== "BUY" && context.direction !== "SELL") {
        decision = "WAIT_RETRACE";
      }
      if (context?.newsFilter?.level === "HIGH") {
        decision = "WAIT_RETRACE";
      }
  ...
  async function maybeSendSignalPush(signal, marketSession, price, m5Analysis, m15Analysis, h1Analysis, signalKey, aiContext, structureTrigger) {
    // Push is deliberately strict: only a NEW CHOCH made by a CLOSED
    // M5/M15 candle, with M5 + M15 aligned in the same direction.
    if (!marketSession?.isOpen || !signal?.confirmed || structureTrigger?.type !== "CHOCH") {
      return {
        attempted: false,
        sent: 0,
        reason: !marketSession?.isOpen
          ? "MARKET_CLOSED"
          : structureTrigger?.type !== "CHOCH"
            ? "NO_NEW_CHOCH"
            : "M5_M15_NOT_ALIGNED"
      };
    }
    ...
    try {
      ...
      let aiAnalysis = await cacheGet(AI_STATE_KEY);
      if (!aiAnalysis || aiAnalysis.signalKey !== signalKey) {
        aiAnalysis = await analyzeWithGemini(aiContext);
        aiAnalysis = { ...aiAnalysis, signalKey };
        await cacheSet(AI_STATE_KEY, aiAnalysis, 15 * 60 * 1000);
      }
      ...
      const zoneLine = aiAnalysis?.status === "OK" && aiAnalysis.entryZone?.low != null && aiAnalysis.entryZone?.high != null
        ...
        : null;
      const body = [
        `NEW CHOCH ${structureTrigger.timeframe} • ${direction}`,
        `Break: ${Number.isFinite(Number(structureTrigger.level)) ? Number(structureTrigger.level).toFixed(2) : "-"}`,
        `Price: ${Number.isFinite(Number(price)) ? Number(price).toFixed(2) : "-"}`,
        `M5/M15: ${m5Analysis?.direction || "-"} / ${m15Analysis?.direction || "-"}`,
        aiLine,
        zoneLine,
  ...
  function buildSignal(m5Analysis, m15Analysis, structureTrigger = null) {
    ...
    if (aligned) {
      const direction = m5;
      const structureText = structureTrigger?.type
        ? ` • ${structureTrigger.type} ${structureTrigger.timeframe}`
        : "";
      return {
        signal: direction,
        direction,
        ...
        confirmed: true,
        reason: `M5 + M15 searah ${direction === "BUY" ? "bullish" : "bearish"}${structureText}`,
        alignment: `M5/M15 ${direction}`,
        structureConfirmed: structureTrigger?.confirmed === true
      };
    }
  ...
  // ---------------------------------------------------------
  // SCALP CHOCH / STRUCTURE TRIGGER
  // A new event exists only when the newest CLOSED candle crosses
  // a confirmed swing that the immediately preceding CLOSED candle
  // had not crossed. This prevents repeat notifications.
  // ---------------------------------------------------------
  function detectNewStructureEvent(candles, swings, structure, timeframe, interval) {
    const closed = getClosedCandles(candles, interval);
    if (closed.length < 3) {
      return { confirmed:false, type:"NONE", timeframe, direction:"NONE", time:null, level:null, price:null };
    }

    const current = closed.at(-1);
    const previous = closed.at(-2);
    const priorHighs = swings.highs.filter(x => x.index < closed.length - 1);
    const priorLows = swings.lows.filter(x => x.index < closed.length - 1);

    // Check the most recent confirmed swing first.
    const high = priorHighs.at(-1) || null;
    const low = priorLows.at(-1) || null;

    if (high && previous.close <= high.price && current.close > high.price) {
      const type = structure.bias === "BEARISH" ? "CHOCH" : "BOS";
      return {
        confirmed:true, type, timeframe, direction:"BULLISH",
        price:current.close, level:high.price, time:current.datetime,
        previousTime:previous.datetime,
        brokenSwing:{price:high.price,time:high.time,index:high.index},
        description:type === "CHOCH"
          ? `Bullish CHOCH: ${timeframe} closed above previous swing high`
          : `Bullish BOS: ${timeframe} closed above previous swing high`
      };
    }

    if (low && previous.close >= low.price && current.close < low.price) {
      const type = structure.bias === "BULLISH" ? "CHOCH" : "BOS";
      return {
        confirmed:true, type, timeframe, direction:"BEARISH",
        price:current.close, level:low.price, time:current.datetime,
        previousTime:previous.datetime,
        brokenSwing:{price:low.price,time:low.time,index:low.index},
        description:type === "CHOCH"
          ? `Bearish CHOCH: ${timeframe} closed below previous swing low`
          : `Bearish BOS: ${timeframe} closed below previous swing low`
      };
    }

    return {
      confirmed:false, type:"NONE", timeframe, direction:"NONE",
      time:current.datetime, previousTime:previous.datetime,
      level:null, price:current.close, description:`No new ${timeframe} structure break`
    };
  }

  function chooseStructureTrigger(m5Event, m15Event) {
    const events = [m5Event, m15Event]
      .filter(x => x?.confirmed)
      .sort((a,b) => candleTimestamp(a.time) - candleTimestamp(b.time));
    return events.at(-1) || {
      confirmed:false, type:"NONE", timeframe:null, direction:"NONE",
      time:null, level:null, price:null, description:"No new M5/M15 structure event"
    };
  }

  function buildScalpSupportResistance(price, m5Swings, m15Swings, trigger) {
    const highs = [...(m15Swings?.highs || []), ...(m5Swings?.highs || [])]
      .filter(x => Number.isFinite(x.price) && x.price > price)
      .sort((a,b) => Math.abs(a.price-price)-Math.abs(b.price-price));
    const lows = [...(m15Swings?.lows || []), ...(m5Swings?.lows || [])]
      .filter(x => Number.isFinite(x.price) && x.price < price)
      .sort((a,b) => Math.abs(a.price-price)-Math.abs(b.price-price));

    let support = lows[0] || null;
    let resistance = highs[0] || null;

    // After a CHOCH, the broken swing is the first retest/reference zone.
    if (trigger?.type === "CHOCH" && Number.isFinite(trigger.level)) {
      if (trigger.direction === "BULLISH" && trigger.level < price) {
        support = { price:trigger.level, time:trigger.time, kind:"BROKEN_SWING_RETEST", timeframe:trigger.timeframe };
      }
      if (trigger.direction === "BEARISH" && trigger.level > price) {
        resistance = { price:trigger.level, time:trigger.time, kind:"BROKEN_SWING_RETEST", timeframe:trigger.timeframe };
      }
    }

    return {
      support: support ? {
        price: round(support.price), time:support.time,
        timeframe:support.timeframe || "M5/M15", kind:support.kind || "NEAREST_SWING_LOW"
      } : null,
      resistance: resistance ? {
        price: round(resistance.price), time:resistance.time,
        timeframe:resistance.timeframe || "M5/M15", kind:resistance.kind || "NEAREST_SWING_HIGH"
      } : null,
      method:"Nearest confirmed M5/M15 swing; CHOCH broken swing becomes retest reference"
    };
  }
  ...
  try {
    ...
    const m5Structure = classifyStructure(m5Swings.highs, m5Swings.lows);
    const m15Structure = classifyStructure(m15Swings.highs, m15Swings.lows);

    const m5Event = detectNewStructureEvent(closedM5, m5Swings, m5Structure, "M5", "5min");
    const m15Event = detectNewStructureEvent(closedM15, m15Swings, m15Structure, "M15", "15min");
    const structureTrigger = chooseStructureTrigger(m5Event, m15Event);

    const signal = buildSignal(m5Analysis, m15Analysis, structureTrigger);
    const now = Date.now();
    const marketSession = getMarketSession(now);
    const newsFilter = await getNewsFilter();

    const scalpSR = buildScalpSupportResistance(
      price, m5Swings, m15Swings, structureTrigger
    );
    ...
    const entryQuality = buildEntryQuality(
      ...
      m5Analysis.atr, scalpSR.support, scalpSR.resistance
    );

    const tradePlan = buildTradePlan(
      signal, price, m5Analysis, scalpSR.support, scalpSR.resistance,
    ...
    const signalKey = structureTrigger.confirmed
      ? `${structureTrigger.type}:${structureTrigger.timeframe}:${structureTrigger.direction}:${structureTrigger.time}`
    ...
    const aiContext = {
      ...
      signalKey,
      structureTrigger,
      scalpSupportResistance: scalpSR,
      ...
      m5: {
        ...
        structure: m5Structure, event: m5Event,
        swings: m5Swings
      },
      m15: {
        ...
        structure: m15Structure, event: m15Event,
        swings: m15Swings
      },
      h1: {
      ...
      supportResistance: {
        support: scalpSR.support?.price ?? null,
        resistance: scalpSR.resistance?.price ?? null,
        supportTime: scalpSR.support?.time ?? null,
        resistanceTime: scalpSR.resistance?.time ?? null,
        h1Support: support?.price ?? null,
        h1Resistance: resistance?.price ?? null,
    ...
    const pushResult = await maybeSendSignalPush(
      signal, marketSession, price, m5Analysis, m15Analysis, h1Analysis,
      signalKey, aiContext, structureTrigger
    );
    ...
    const marketFilter = {
      ...
      note: !marketSession.isOpen
        ? "Market closed. Notifications are paused."
        : structureTrigger.confirmed
          ? `${structureTrigger.type} detected on ${structureTrigger.timeframe}; AI evaluates entry after the closed candle.`
          : "Wait for a new M5/M15 CHOCH or aligned scalp setup."
    };
    ...
    return res.status(200).json({
      ...
      structure: {
        ...
        m5: m5Event,
        m15: m15Event,
        focus: "M5 + M15 CHOCH/BOS on CLOSED candle; previous-candle crossing required"
      },
      ...
      supportResistance: {
        method: scalpSR.method,
        support: scalpSR.support?.price ?? null,
        supportTime: scalpSR.support?.time ?? null,
        supportTimeframe: scalpSR.support?.timeframe ?? null,
        supportKind: scalpSR.support?.kind ?? null,
        resistance: scalpSR.resistance?.price ?? null,
        resistanceTime: scalpSR.resistance?.time ?? null,
        resistanceTimeframe: scalpSR.resistance?.timeframe ?? null,
        resistanceKind: scalpSR.resistance?.kind ?? null,
        h1Support: support ? round(support.price) : null,
        h1Resistance: resistance ? round(resistance.price) : null,
      ...
      engine: {
        timeframe: "M5 + M15 + H1",
        scalpLogic: "M5 and M15 closed candles must align; new structure event is based on previous-vs-current closed candle crossing",
        h1Logic: "H1 is context/hold",
        supportResistance: "Nearest confirmed M5/M15 swing highs/lows; the CHOCH broken swing is the retest reference",
        ...
        timezone: "UTC",
        aiTrader: "Gemini second-layer setup analyst; evaluates only after a new closed M5/M15 structure event and returns WAIT when entry is not suitable",
