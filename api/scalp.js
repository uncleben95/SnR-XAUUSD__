...
async function td(interval,outputsize){
  const u=new URL(BASE); u.searchParams.set("symbol",CFG.symbol); u.searchParams.set("interval",interval); u.searchParams.set("outputsize",outputsize); u.searchParams.set("apikey",TD_KEY); u.searchParams.set("timezone","UTC");
...
export default async function handler(req,res){
  try{
    ...
    const [m5raw,m15raw,h1raw]=await Promise.all([td("5min",CFG.outputM5),td("15min",CFG.outputM15),td("1h",CFG.outputH1)]);
    const m5=closedRows(m5raw,5), m15=closedRows(m15raw,15), h1=closedRows(h1raw,60);
    if(m5.length<80||m15.length<40||h1.length<20) throw new Error("Insufficient closed market data");
