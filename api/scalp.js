...
export default async function handler(req, res) {
  ...
  function structureNone(description = "No structure break detected") {
    return {
      type: "NONE",
      direction: "NONE",
      price: null,
  ...
  // Detect the MOST RECENT confirmed BOS/CHOCH by looking at
  // candle-to-candle breaks. A break is only allowed to use a
  // swing that was already confirmed before the break candle.
  function detectStructureEvent(
    ...
    let latest = structureNone();

    for (let i = 1; i < candles.length; i++) {
      const current = candles[i];
      const previous = candles[i - 1];

      const confirmedHighs = swingHighs.filter(
        x => x.index + pivotRight < i
      );

      const confirmedLows = swingLows.filter(
        x => x.index + pivotRight < i
      );

      const high = confirmedHighs.at(-1);
      const low = confirmedLows.at(-1);

      if (high && previous.close <= high.price && current.close > high.price) {
        const beforeStructure = classifyStructure(
          confirmedHighs,
          confirmedLows
        );

        const type =
          beforeStructure.highPattern === "LH"
            ? "CHOCH"
            : "BOS";

        const protectedLow = confirmedLows
          .filter(x => x.index < high.index)
          .at(-1) || confirmedLows.at(-1) || null;
        ...
        latest = {
          ...
          candleIndex: i,
          brokenSwing: high,
          protectedSwing: protectedLow,
          description:
            type === "CHOCH"
              ? "Bullish CHOCH - bearish swing high broken"
      ...
      if (low && previous.close >= low.price && current.close < low.price) {
        ...
        const type =
          beforeStructure.lowPattern === "HL"
            ? "CHOCH"
            : "BOS";

        const protectedHigh = confirmedHighs
          .filter(x => x.index < low.index)
          .at(-1) || confirmedHighs.at(-1) || null;
        ...
        latest = {
          ...
          candleIndex: i,
          brokenSwing: low,
          protectedSwing: protectedHigh,
          description:
            type === "CHOCH"
              ? "Bearish CHOCH - bullish swing low broken"
  ...
  // H1 S/R is structure-aware:
  // - do NOT use the nearest micro swing by default
  // - prefer the protected swing created by the latest BOS/CHOCH
  // - require meaningful ATR-based distance from current price
  // - otherwise fall back to the nearest meaningful H1 swing
  function selectStructuralLevels(
    price,
    h1Highs,
    h1Lows,
    h1ATR,
    ...
    const atrValue = Number(h1ATR) || 0;
    const minDistance = Math.max(atrValue * 0.35, 1.0);

    const below = findStructuralCandidates(
      h1Lows,
      price,
      "below",
      minDistance
    ...
    const above = findStructuralCandidates(
      h1Highs,
      price,
      "above",
      minDistance
    ...
    let support = below[0] || null;
    let resistance = above[0] || null;

    const event = structureEvent;

    if (event?.direction === "BULLISH") {
      // The last protected low before the bullish break is the
      // primary structural support if it remains below price.
      const protectedLow = event.protectedSwing;

      if (
        protectedLow &&
        protectedLow.price < price - minDistance
      ) {
        support = protectedLow;
      }

      // After a bullish break, prefer a meaningful swing high
      // formed after the break as resistance. If none exists,
      // use the next meaningful H1 swing above price.
      const postBreakHigh = h1Highs
        .filter(
          x =>
            x.index > event.candleIndex &&
            x.price > price + minDistance
        )
        .sort((a, b) => a.price - b.price)[0];

      if (postBreakHigh) {
        resistance = postBreakHigh;
      }
    }

    if (event?.direction === "BEARISH") {
      // The last protected high before the bearish break is the
      // primary structural resistance if it remains above price.
      const protectedHigh = event.protectedSwing;

      if (
        protectedHigh &&
        protectedHigh.price > price + minDistance
      ) {
        resistance = protectedHigh;
      }

      // After a bearish break, prefer a meaningful swing low
      // formed after the break as support. Otherwise use the next
      // meaningful H1 swing below price.
      const postBreakLow = h1Lows
        .filter(
          x =>
            x.index > event.candleIndex &&
            x.price < price - minDistance
    ...
    return {
      ...
      minDistance,
      method:
        "H1 structure-aware BOS/CHOCH protected swing + ATR filter"
    };
  }

  function latestBelow(swings, price) {
    const levels = swings.filter(
      x => Number.isFinite(x.price) && x.price < price
    );
  ...
  try {
    ...
    const support =
      structuralLevels.support;

    const resistance =
      structuralLevels.resistance;

    // Previous structural levels, not merely the nearest micro swing.
    const supportCandidates =
      findStructuralCandidates(
        h1Lows,
        price,
        "below",
        structuralLevels.minDistance
      );
    ...
    return res.status(200).json({
      ...
      h1: {
        ...
        structureMethod:
          "H1 BOS/CHOCH + protected swing + ATR filter",
      ...
      filters: {
        ...
        supportResistance:
          "H1 BOS/CHOCH structural swings with ATR distance filter",
