...
export default async function handler(req, res) {
  ...
  try {
     ...
     * ==========================================================
     *
     * Push notification is tied ONLY to a NEW BUY/SELL signal.
     * CHOCH remains available for analysis/display but is NOT a
     * notification trigger.
     *
     * Signal requirements are decided by the existing signal engine:
     * - M15 + M5 alignment
     * - H1 bias/hold rules
     * - S/R filters
     * - roadblocks
     * - existing score/setup rules
     *
     * Redis dedup prevents the same signal candle from being pushed
     * repeatedly by the 5-minute cron.
     */
    ...
    async function notifySignal(signalDirection, key, candleTime) {
      if (!key || !candleTime) return;

      const lockKey =
        `xau_signal_notification|${key}`;
      ...
      try {
        ...
        const emoji = isBuy ? "🟢" : "🔴";
        const title =
          `${emoji} XAU/USD ${signalDirection} SIGNAL`;

        const price =
          Number(livePrice).toFixed(2);

        const body = [
          `${signalDirection} signal terbentuk`,
          `Price ${price}`,
          `Candle ${candleTime}`,
          `H1 ${h1Direction}`,
          `Score ${score}`
        ...
        console.error(
          `SIGNAL push error [${signalDirection}]:`,
    ...
    /*
     * Only a real BUY/SELL signal can trigger push.
     * WAIT never triggers push.
     */
    if (
      signalKey &&
    ...
    return res.status(200).json({
      ...
      version:
        "V16-SCALP-SIGNAL-PUSH",

      architecture:
        "M15+M5-ALIGNED-SIGNAL + H1-HOLD + H1-SR-TARGET + M15/M5-ROADBLOCK + FIXED-ENTRY-SL-TP + SIGNAL-BUY-SELL-PUSH + TIMEFRAME-CACHE + LIVE-PRICE",
