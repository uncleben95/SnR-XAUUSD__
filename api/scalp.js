...
Market data:\n${JSON.stringify(context)}`;
  ...
  function buildTradePlan(signal, price, m5, support, resistance, h1Liquidity, news, marketSession = { isOpen: true }) {
    if (!marketSession.isOpen || signal.direction === "NEUTRAL") {
      return {
        ...
        riskDistance: null, newsRisk: news?.level || "UNKNOWN", reason: signal.reason,
    ...
    if (signal.direction === "BUY") {
      ...
      const h1Target = h1Liquidity?.price && h1Liquidity.price > price ? h1Liquidity.price : null;
      const tp1 = h1Target || price + risk * 1.2;
      const tp2 = h1Target ? tp1 + Math.max(risk * 0.8, safeATR) : price + risk * 2;
      const tp3 = h1Target ? tp1 + Math.max(risk * 1.5, safeATR * 1.5) : price + risk * 2.5;
      return {
        status: "BUY",
        direction: "BUY",
        entry: round(price),
        ...
        newsRisk: news.level,
        liquidityTarget: h1Target ? { type: "H1_SWING_HIGH_LIQUIDITY", price: round(h1Target), time: h1Liquidity.time } : null,
    ...
    return {
      ...
      riskDistance: round(risk),
      newsRisk: news.level,
      liquidityTarget: h1Target ? { type: "H1_SWING_LOW_LIQUIDITY", price: round(h1Target), time: h1Liquidity.time } : null,
  ...
  try {
    ...
    const h1LiquidityTarget = signal.direction === "BUY"
      ? nearestAbove(h1Highs, price)
      : signal.direction === "SELL"
        ? nearestBelow(h1Lows, price)
        : null;

    const tradePlan = buildTradePlan(
      signal, price, m5Analysis, support, resistance,
      h1LiquidityTarget, newsFilter, marketSession
    );
    ...
    return res.status(200).json({
      ...
      filters: {
        ...
        h1: "Confirmed swing high/low = liquidity zones / sweep targets",
        h1Conflict: "Does not block M5/M15 scalp signal",
