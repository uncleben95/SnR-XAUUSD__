...
export default async function handler(req, res) {
  ...
  try {
     ...
     * ==========================================================
     *
     * PUSH IS NOW COMPLETELY INDEPENDENT FROM ENTRY.
     *
     * A notification is sent ONLY when a NEW CHOCH event is
     * detected on the newest CLOSED candle of M5 or M15.
     *
    ...
    async function notifyChochEvent(timeframe, direction, event) {
      ...
      if (!event.bullish && !event.bearish) {
        return;
      }
      ...
      const eventKey =
        `XAUUSD|${timeframe}|CHOCH|${direction}|${candleTime}`;

      const lockKey =
        `xau_choch_notification|${eventKey}`;
      ...
      try {
         ...
         *
         * Old successful pushes were stored as plain "1".
         * Remove that old-format lock once so the existing
         * M15/M5 test events can be retried after deployment.
         * New successful pushes use version 2 and remain protected
         * against duplicate GitHub Actions runs.
         */
        if (alreadyNotified) {
          let legacyLock = false;

          if (alreadyNotified === "1" || alreadyNotified === 1) {
            legacyLock = true;
          } else if (typeof alreadyNotified === "string") {
            try {
              const parsed = JSON.parse(alreadyNotified);
              legacyLock = parsed?.version !== 2;
            } catch {
              legacyLock = true;
            }
          } else if (typeof alreadyNotified === "object") {
            legacyLock = alreadyNotified?.version !== 2;
          }

          if (legacyLock) {
            await redis.del(lockKey);
          } else {
            chochPushSkipped = true;

            chochPushEvents.push({
              timeframe,
              direction,
              candle: candleTime,
              key: eventKey,
        ...
        const title =
          `${emoji} XAU/USD ${timeframe} CHOCH`;

        const price =
          Number(event.candle.close).toFixed(2);
        ...
        if (sent) {
          /*
           * Only a successful push creates a new lock.
           */
          await redis.set(
            lockKey,
            JSON.stringify({
          ...
          /*
           * No lock is created if delivery fails or there are
           * no subscribers. The next scheduled run can retry.
           */
          chochPushEvents.push({
            timeframe,
            direction,
            candle: candleTime,
            key: eventKey,
            status: "NO_SUBSCRIBERS_OR_NOT_SENT",
            sent: Number(delivery?.sent || 0),
            removed: Number(delivery?.removed || 0),
            failed: Number(delivery?.failed || 0),
            total: Number(delivery?.total || 0)
        ...
        console.error(
          `CHOCH push error [${timeframe} ${direction}]:`,
          pushError?.message || pushError
        );

        /* Never create a lock when the push operation errors. */
        chochPushEvents.push({
          timeframe,
          direction,
          candle: candleTime,
    ...
    /*
     * PUSH RULE:
     * Notify when the LATEST CLOSED candle is in a CHOCH state.
     * Redis deduplication ensures the same timeframe/direction/
     * candle is only pushed once.
     *
     * This intentionally does NOT require a false -> true transition
     * in the state detector. That way, if the dashboard shows CHOCH
     * on the latest closed candle, the phone gets the notification.
     */
    const latestM15 = m15.at(-1);
    const latestM5 = m5.at(-1);

    if (m15CHOCH.bullish && latestM15) {
      await notifyChochEvent(
        "M15",
        "BULLISH",
        {
          candle: latestM15,
    ...
    /*
     * Keep the old push fields for frontend compatibility, but
     * they now represent CHOCH push status rather than ENTRY push.
     */

    let pushSent = chochPushSent;
    let pushSkipped = chochPushSkipped;
    ...
    return res.status(200).json({
      ...
      version:
        "V15-SCALP-CLOSED-CANDLE-CHOCH-PUSH",

      architecture:
        "M15+M5-ALIGNED-SIGNAL + H1-HOLD + H1-SR-TARGET + M15/M5-ROADBLOCK + FIXED-ENTRY-SL-TP + CLOSED-CANDLE-CHOCH-PUSH + TIMEFRAME-CACHE + LIVE-PRICE",
