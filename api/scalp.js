...
export default async function handler(req, res) {
  ...
  try {
    ...
    // SINGLE SOURCE OF TRUTH: marketSession
    // Both Market Filter and Market Session must use this same state.
    ...
    const marketFilter = {
      status:
        marketStatusLabel === "MARKET_CLOSED"
          ? "MARKET_CLOSED"
          : signal.confirmed
          ? "SCALP_READY"
          : "WAIT",
    ...
    return res.status(200).json({
      ...
      // Compatibility aliases: any frontend card reading
      // market / marketSession / marketFilter gets the same state.
      market: {
        ...marketSession,
