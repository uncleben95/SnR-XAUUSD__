...
export default async function handler(req, res) {
  ...
  try {
     ...
     * ==========================================================
     *
     * Push notification kembali ikut SIGNAL BUY/SELL.
     *
     * Background trigger datang daripada GitHub Actions setiap
     * 5 minit, jadi notification tidak bergantung kepada dashboard
     * sedang dibuka atau tidak.
     *
     * Redis dedup memastikan signal + candle yang sama hanya
     * dihantar sekali.
     */
    ...
    async function notifySignal() {
      ...
      const lockKey =
        `xau_signal_notification|${signalKey}`;
      ...
      try {
        ...
        if (entry != null) {
          parts.push(`Entry ${Number(entry).toFixed(2)}`);
        }

        if (stopLoss != null) {
          parts.push(`SL ${Number(stopLoss).toFixed(2)}`);
        }

        if (tp1 != null) {
          parts.push(`TP1 ${Number(tp1).toFixed(2)}`);
        }
