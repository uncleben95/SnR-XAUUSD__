...
export default async function handler(req,res){
  try{
    ...
    // Fetch real M5 + M15 + H1 series. Redis caching reduces provider calls.
    // M5/M15 are both primary scalp timeframes; H1 is context only.
    const [m5raw,m15raw,h1raw]=await Promise.all([
      td("5min",CFG.outputM5,"xau:td:m5",5*60*1000),
      td("15min",CFG.outputM15,"xau:td:m15",15*60*1000),
      td("1h",CFG.outputH1,"xau:td:h1",60*60*1000)
    ]);

    const m5=closedRows(m5raw,5);
    const m15=closedRows(m15raw,15);
    ...
    // H1 liquidity is kept separate from scalp S/R. These are the latest
    // confirmed H1 swing high/low and are context/possible sweep zones only.
    const h1Liquidity={
      buySideLiquidity:h1r.high,
      sellSideLiquidity:h1r.low,
      swingHigh:h1r.high,
      swingLow:h1r.low,
      reference:"Latest confirmed H1 swing high/low",
      role:"LIQUIDITY_ONLY"
    };
    ...
    // Market-closed guard: no new breakout, retest, entry, or push outside the XAU/USD session.
    if(session.status!=="OPEN") {
      decision={action:"WAIT",reason:`Market closed — ${session.reason}. No new scalp setup or push will be generated.`};
    }

    // Create persistent breakout state only from a fresh closed M5 candle crossing an existing S/R.
    ...
    return res.status(200).json({
      ...
      entry,
      signal,
      data:{m5Candles:m5.length,m15Candles:m15.length,h1Candles:h1.length,h1RangeHigh:h1r.high?.price??null,h1RangeLow:h1r.low?.price??null},
