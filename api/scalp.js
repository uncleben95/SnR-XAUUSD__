...
Market data:\n${JSON.stringify(context)}`;
  ...
  function detectClosedCHOCH(candles, swings) {
    ...
    const last = candles.at(-1);
    if (!last) return { type:"NONE", direction:"NONE", price:null, level:null, time:null, candleIndex:lastIndex };

    const prior = structureBeforeIndex(candles, swings, lastIndex);
    const lastHigh = prior.highs.at(-1);
    const lastLow = prior.lows.at(-1);

    if (lastHigh && last.close > lastHigh.price && prior.bias === "BEARISH") {
      return {
        ...
        description:"Bullish CHOCH: closed candle broke the previous bearish structure high"
      };
    }

    if (lastLow && last.close < lastLow.price && prior.bias === "BULLISH") {
      return {
        ...
        previousStructure: { bias:prior.bias, highPattern:prior.highPattern, lowPattern:prior.lowPattern },
        description:"Bearish CHOCH: closed candle broke the previous bullish structure low"
      };
    }

    return { type:"NONE", direction:"NONE", price:null, level:null, time:last.datetime, candleIndex:lastIndex,
      previousStructure:{ bias:prior.bias, highPattern:prior.highPattern, lowPattern:prior.lowPattern },
  ...
  try {
    ...
    return res.status(200).json({
      ...
      h1: {
        ...
        h1SupportResistanceMethod: h1StructuralLevels.method,

        support: h1StructuralLevels.support ? {
          price: round(h1StructuralLevels.support.price),
          time: h1StructuralLevels.support.time,
          distance: round(price - h1StructuralLevels.support.price)
        } : null,

        resistance: h1StructuralLevels.resistance ? {
          price: round(h1StructuralLevels.resistance.price),
          time: h1StructuralLevels.resistance.time,
          distance: round(h1StructuralLevels.resistance.price - price)
        } : null,

        previousSupport: h1StructuralLevels.support ? {
          price: round(h1StructuralLevels.support.price),
          time: h1StructuralLevels.support.time
        } : null,

        previousResistance: h1StructuralLevels.resistance ? {
          price: round(h1StructuralLevels.resistance.price),
          time: h1StructuralLevels.resistance.time
        } : null,
