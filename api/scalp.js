...
export default async function handler(req, res) {
  ...
  try{
    ...
    const rawBreak=buy15 && !sell15 ? buy15 : sell15 && !buy15 ? sell15 : buy5 && !sell5 ? buy5 : sell5 && !buy5 ? sell5 : null;
    ...
    if(rawBreak){
      ...
      const after=m5.filter(c=>Date.parse(String(c.time).replace(" ","T")+"Z")>breakTime);
    ...
    const signalKey=(direction!=="WAIT" && rawBreak)
      ? `XAUUSD|SCALP_ENTRY|${direction}|${rawBreak.level.price}|${rawBreak.candle.time}`
      : null;
