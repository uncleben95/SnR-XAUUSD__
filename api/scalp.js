...
export default async function handler(req,res){
  try{
    ...
    const [m5raw,h1raw]=await Promise.all([
      td("5min",CFG.outputM5,"xau:td:m5",CFG.m5CacheTTL),
      td("1h",CFG.outputH1,"xau:td:h1",CFG.h1CacheTTL)
    ]);
    ...
    // M5 is the execution trigger. M15/H1 are context only.
    if(session.status==="OPEN" && (s5.event==="BOS_UP"||s5.event==="BOS_DOWN"||s5.event==="CHOCH_UP"||s5.event==="CHOCH_DOWN")){
      const side=s5.side;
      quality=breakoutQuality(m5,m15,h1s,s5,s15,side);
      entry=buildTrade(m5,m15,s5,side,quality);
      breakout={
        confirmed:true,side,price:s5.level.price,
        sourceTimeframe:"M5",structureEvent:s5.event,
        level:{price:s5.level.price,type:side==="BUY"?"SWING_HIGH":"SWING_LOW"},
        candle:{time:s5.candle.time},
        distanceFromCurrent:price-s5.level.price
      };

      if(quality.score>=CFG.minScore){
        decision={action:"ENTRY_READY",reason:`Fresh M5 ${s5.event} breakout. No retest required. ${quality.grade} setup ${quality.score}/100.`};
    ...
    const h1Context={
      ...
      alignment:s5.direction===h1s.direction&&s15.direction===h1s.direction?"ALIGNED":s5.direction===h1s.direction||s15.direction===h1s.direction?"MIXED":"OPPOSED"
    };

    return res.status(200).json({
      ok:true,engine:"XAUUSD-SCALP-PRO-M5-BREAKOUT",timestamp:new Date().toISOString(),
      marketStatus:session.status,marketReason:session.reason,price,scalpOnly:true,
      ...
      rules:{
        primary:"M5 breakout execution",
        supportResistance:"Confirmed M5/M15 swing structure",
        breakout:"Fresh closed M5 BOS/CHOCH",
        entry:"Breakout candle close — NO RETEST REQUIRED",
        minimumScore:CFG.minScore,minimumRR:CFG.minRR,
        h1:"Context only; never blocks M5 scalp"
      },
      push
    });
  }catch(e){
    ...
    return res.status(status).json({ok:false,error:msg,hint:status===503?"Twelve Data rate limit; cached M5/H1 data will be used after a successful fetch.":undefined});
  }
}
