...
export default async function handler(req, res) {
  ...
  try {
    ...
    if(status==="ENTRY"&&m5ATR!=null){
      entry=livePrice;
      const risk=Math.max(m5ATR*1.25,0.8);
      if(signal==="BUY"){stopLoss=entry-risk;tp1=entry+risk*1.5;tp2=entry+risk*2.5;tp3=entry+risk*4}
      else {stopLoss=entry+risk;tp1=entry-risk*1.5;tp2=entry-risk*2.5;tp3=entry-risk*4}
      rr="1 : 1.5 / 2.5 / 4.0";
    }
