...
Rules:
- The technical engine calls you after a NEW CLOSED M5 CHOCH. M15 is context only and does NOT block the alert.
...
Market data:\n${JSON.stringify(context)}`;
  ...
  function buildSignal(m5Analysis, m15Analysis, m5Event, m15Event) {
    ...
    // NEW RULE:
    // A NEW confirmed CLOSED-M5 CHOCH is the alert trigger by itself.
    // M15 is still displayed/analyzed, but it does NOT gate the notification.
    // H1 remains context only.
    if (m5Event?.type === "CHOCH" && (m5Event.direction === "BULLISH" || m5Event.direction === "BEARISH")) {
      ...
      return {
        ...
        m15Confirmation: m15Aligned ? "ALIGNED" : "NOT_ALIGNED_BUT_NOT_BLOCKING",
        reason: `M5 closed-candle CHOCH ${direction === "BUY" ? "bullish" : "bearish"} — immediate alert`,
        alignment: m15Aligned ? `M5/M15 ${direction}` : `M5 ${direction} / M15 ${m15 || "NEUTRAL"}`
      };
    }
    ...
    return {
      ...
      triggerEvent: null, m15Confirmation: "NOT_REQUIRED",
      reason: "Tiada CHOCH baru pada candle M5 yang sudah closed",
      alignment: m5 === m15 && m5 !== "NEUTRAL"
        ? `M5/M15 ${m5} - WAIT CHOCH`
        : `M5 ${m5 || "NEUTRAL"} - WAIT CHOCH`
    };
  }
  ...
  try {
    ...
    const marketFilter = {
      ...
      note: !marketSession.isOpen
        ...
        : "M5 CHOCH is the trigger; M15 is context only and does not block. H1 is context only."
    };
    ...
    return res.status(200).json({
      ...
      engine: {
        timeframe: "M5 + M15 + H1",
        scalpLogic: "NEW CLOSED M5 CHOCH = immediate alert; M15 context only",
