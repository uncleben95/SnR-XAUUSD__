...
//
// Push notifications are triggered server-side when this endpoint is
// called by GitHub Actions / another backend scheduler. The push module
// is loaded lazily so a missing push configuration never breaks the
// market-data API.
...
export default async function handler(req, res) {
  ...
  // ---------------------------------------------------------
  // MARKET SESSION + BACKGROUND PUSH
  // ---------------------------------------------------------
  ...
  function getMarketSession(now = new Date()) {
    ...
    // Standard XAU/USD weekend protection. Broker/venue hours can vary,
    // so this deliberately focuses on the broad weekend closure.
    if (day === 6) {
      return {
        open: false,
    ...
    // Sunday before the normal global precious-metals reopen window.
    if (day === 0 && totalMinutes < 21 * 60) {
      return {
        open: false,
        status: "MARKET_CLOSED",
        reason: "Weekend",
    ...
    // Friday after the normal global close window.
    if (day === 5 && totalMinutes >= 21 * 60) {
      return {
        open: false,
        status: "MARKET_CLOSED",
        reason: "Weekend",
  ...
  async function maybeSendSignalPush(signal, marketSession, price, m5Analysis, m15Analysis, entryQuality, tradePlan) {
    if (!marketSession.open) {
      // Do not carry Friday's signal into the next trading session.
      await redisCommand(["DEL", "xau:push:last-direction"]);
      return {
        attempted: false,
        sent: 0,
    ...
    const direction = signal?.confirmed
      ? String(signal.direction || "").toUpperCase()
      : "WAIT";

    // Only BUY/SELL transitions generate a notification.
    if (direction !== "BUY" && direction !== "SELL") {
      // Reset the last-notified direction so a later BUY/SELL is a new event.
      await redisCommand(["DEL", "xau:push:last-direction"]);
      return {
        attempted: false,
        sent: 0,
    ...
    const lastDirection = await redisCommand([
      "GET",
      "xau:push:last-direction"
    ]);
    ...
    // Persist the direction before delivery to avoid duplicate sends when
    // GitHub Actions overlaps/retries the same five-minute run.
    await redisCommand([
      "SET",
      "xau:push:last-direction",
      direction,
    ...
    try {
      ...
      const score = Number(entryQuality?.score ?? 0);
      const body =
        `${direction} • XAU/USD ${Number(price).toFixed(2)} • ` +
        `M5 ${m5Analysis?.direction || "-"} / M15 ${m15Analysis?.direction || "-"} • ` +
        `Entry ${tradePlan?.entry ?? Number(price).toFixed(2)} • ` +
        `SL ${tradePlan?.stopLoss ?? "-"} • TP1 ${tradePlan?.takeProfit1 ?? "-"} • ` +
        `Quality ${score}`;

      const result = await sendPushToAll({
        title: `XAU/USD ${direction} SIGNAL`,
        body,
  ...
  try {
    ...
    if (!marketSession.open) {
      ...
      signal.scalpSignal = "WAIT";
      signal.signal = "WAIT";
      signal.reason = `Market closed: ${marketSession.reason}`;
    }
    ...
    const marketFilter = {
      ...
      marketSession: marketSession.status,

      note:
        !marketSession.open
          ? `Market closed: ${marketSession.reason}. No scalp notification is sent.`
    ...
    return res.status(200).json({
      ...
      engine: {
        ...
        pushNotifications:
          "Server-side on new BUY/SELL transition",
