...
const CFG = {
  ...
  eventSeenPrefix: "xau:scalp:v40:event-seen:"
};
...
export default async function handler(req,res){
  try{
    ...
    // PRIMARY TRIGGER: a fresh confirmed M5 or M15 BOS/CHOCH event.
    // The event's broken structure level becomes the retest level.
    // H1 is context/liquidity only and never blocks the M5/M15 scalp.
    const last=m5.at(-1);
    if(session.status==="OPEN" && !pending){
      const events=[];
      if((s5.event==="BOS_UP"||s5.event==="BOS_DOWN") && s5.level){
        events.push({tf:"M5",event:s5.event,side:s5.event==="BOS_UP"?"BUY":"SELL",level:s5.level,candle:s5.candle});
      }
      if((s15.event==="BOS_UP"||s15.event==="BOS_DOWN") && s15.level){
        events.push({tf:"M15",event:s15.event,side:s15.event==="BOS_UP"?"BUY":"SELL",level:s15.level,candle:s15.candle});
      }
      // Prefer the M5 event when both timeframes print an event on the same run.
      const ev=events.find(x=>x.tf==="M5")||events[0];
      if(ev){
        const eventKey=`${ev.tf}|${ev.event}|${ev.side}|${ev.level.price}|${ev.candle.time}`;
        const eventLock=CFG.eventSeenPrefix+eventKey.replace(/[^A-Za-z0-9_.:-]/g,"_");
        let seen=false;
        try{ seen=(await redis.get(eventLock))==="1"; }catch{}
        if(!seen){
          try{ await redis.set(eventLock,"1",{ex:86400}); }catch{}
          pending={
            side:ev.side,
            level:ev.level.price,
            breakoutCandle:ev.candle.time,
            breakoutIndex:m5.findIndex(x=>x.time===ev.candle.time),
            sourceTimeframe:ev.tf,
            structureEvent:ev.event,
            createdAt:Date.now()
          };
    ...
    if(pending){
      const bi=m5.findIndex(x=>x.time===pending.breakoutCandle);
      breakout={confirmed:true,side:pending.side,price:pending.level,sourceTimeframe:pending.sourceTimeframe||"M5",structureEvent:pending.structureEvent||"BOS",level:{price:pending.level,type:pending.side==="BUY"?"SWING_HIGH":"SWING_LOW"},candle:{time:pending.breakoutCandle},distanceFromCurrent:price-pending.level};
      let retestIndex=-1;
    ...
    return res.status(200).json({
      ...
      rules:{primary:"M5 + M15 scalp",supportResistance:"Confirmed M5/M15 structure",breakout:"Fresh closed M5/M15 BOS/CHOCH; broken structure becomes retest level",entry:"BOS/CHOCH -> M5 retest -> M5 continuation",minimumRR:CFG.minRR,h1:"Context only; never blocks scalp"},
      push
    });
  }catch(e){
    console.error(e);
